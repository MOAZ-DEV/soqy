...
export const app = new Hono()
	...
	.get("/products", async (c) => {
		try {
			...
			const { products, pageInfo, cursors } = await shopifyAPI.getProducts(params);
			return c.json({ success: true, products, pageInfo, cursors });
		} catch (error: any) {
			console.error("/products error:", error);
			return c.json({ success: false, error: error?.message || String(error) }, { status: 500 });
		}
	})
	...
	.get("/product/:handle", async (c) => {
		...
		try {
			...
			if (!product) {
				return c.json({ success: false, message: "Product not found" }, { status: 404 });
			}
	...
	.get("/collection/:handle", async (c) => {
		...
		try {
			...
			if (!collection) {
				return c.json({ success: false, message: "Collection not found" }, { status: 404 });
			}
	...
	.get("/collection/:handle/products/:sortkey", async (c) => {
		...
		try {
			const collection = await shopifyAPI.getAllCollectionProducts(handle, sortkey as ProductSortKeys);
			if (!collection) {
				return c.json({ success: false, message: "Collection not found" }, { status: 404 });
			}
			return c.json({ success: true, collection });
	...
	.post("/cart/add", async (c) => {
		...
		try {
			const cart = await shopifyAPI.addToCart(cartId, [{ merchandiseId: variantId, quantity }]);
			return c.json({ success: true, cart });
		} catch (error) {
			return c.json({ success: false, error }, { status: 500 });
		}
	...
	.post("/cart/update", async (c) => {
		...
		try {
			const cart = await shopifyAPI.updateCart(cartId, [{ id: lineId, quantity }]);
			return c.json({ success: true, cart });
		} catch (error) {
			return c.json({ success: false, error }, { status: 500 });
		}
